...
def rule_1(source, end, start):
    if start.minute < end.minute:
        ...
        total_chamada = ((end.hour - start.hour) * MINUTES_IN_AN_HOUR) - (start.minute - end.minute)
    price = (total_chamada * VALUE_PER_MINUTE) + FIXED_VALUE
    currente_value = dict_check(source)
    if currente_value != None:
        price = price + currente_value 
    resultado_parcial = {}
    resultado_parcial[source] = price 
    final_result.update(resultado_parcial) 
       
def rule_2(source, end, start):
    total_chamada = ((NIGHT_HOUR - start.hour) * MINUTES_IN_AN_HOUR) - start.minute
    price = (total_chamada * VALUE_PER_MINUTE) + FIXED_VALUE
    currente_value = dict_check(source)
    if currente_value != None:
        price = price + currente_value 
...
def rule_3(source, end, start):    
    total_chamada = ((end.hour - DAY_HOUR) * MINUTES_IN_AN_HOUR) + end.minute
    price = (total_chamada * VALUE_PER_MINUTE) + FIXED_VALUE    
    currente_value = dict_check(source)
    if currente_value != None:
        price = price + currente_value   
...
def charging_rule(source, end, start):
    if start.hour >= DAY_HOUR and start.hour < NIGHT_HOUR:
        if (end.hour >= DAY_HOUR and end.hour < NIGHT_HOUR) or (end.hour == NIGHT_HOUR and end.minute == 0):
        ...
        if (end.hour >= DAY_HOUR and end.hour < NIGHT_HOUR) or (end.hour == NIGHT_HOUR and end.minute == 0):
            ...
            if (end.hour >= start.hour and end.hour < DAY_HOUR) or (end.hour == DAY_HOUR and end.minute == 0):
                rule_4(source)             
...
def finale():
    dict_aux = {}
    record = []             
    ordered_result = OrderedDict(sorted(final_result.items(), key=lambda x: x[1]))
